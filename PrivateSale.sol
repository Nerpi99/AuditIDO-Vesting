...
contract PrivateSale is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    // Array to register all the buyers
    ...
    modifier not0Address(address _address){
        _not0Address(_address);
        _;
    }
    ...
    function initialize(address payable _wallet,uint256 _openingTime, uint256 _closingTime, uint256 _cap, uint256 initialPrice,address _vestingContract, address _rolesContract) public initializer not0Address(_wallet) not0Address(_vestingContract) not0Address(_rolesContract) {
    ...
    function _not0Address(address _address) internal {
    ...
    function setOracle(address _newOracle) external onlyOwner whenPaused not0Address(_newOracle) {
    ...
    function setToken(address _newToken) external onlyOwner whenPaused not0Address(_newToken) {
    ...
    function setRouter(address _newRouter) external onlyOwner whenPaused not0Address(_newRouter) {
    ...
    function setRoles(address _newRoles) external onlyOwner whenPaused not0Address(_newRoles){
        rolesContract = IRoles(_newRoles);
    }

    /// @dev The value is in USDC
    /// @dev the dimension is in weis (newValue * 10**18)
    /// @param _newCap new max amount to raise
    function setCap(uint256 _newCap) external onlyOwner {
        require(_newCap > 0, "New cap should be greater than 0");
        require(_newCap >= usdcRaised, "The new cap should be greater or equal than the USD already raised"); //if equal, the sells stops
        cap = _newCap;
    }
     ...
     */
    function buyTokens(address _beneficiary) public nonReentrant whenNotPaused not0Address(_beneficiary) payable {
    ...
    /// @param _tokenAmount the amount of token to vest
    /// @param _usdcPaid the amount of usdc paid
    function privateSell(address _beneficiary, uint256 _tokenAmount, uint256 _usdcPaid) public onlyOwner nonReentrant not0Address(_beneficiary) {
        usdcRaised += _usdcPaid;
        require(!this.capReached(), "The cap is exceeded");
        tokenSold += _tokenAmount;
        _updateScheduleAmount(_beneficiary, _tokenAmount);
        if(alreadyInvested[_beneficiary] == 0){
            investors.push(_beneficiary);
        }
        alreadyInvested[_beneficiary] += _usdcPaid;
    }
    ...
    /// @dev Function to get all the addresses that buy in this stage
    /// @return investors
    ...
    function _updatePurchasingState(address beneficiary, uint256 weiAmount) internal {
        ...
        if(alreadyInvested[beneficiary] == 0){
    ...
    function _extendTime(uint256 newClosingTime) internal {
        ...
        require(newClosingTime >= block.timestamp, "TimedCrowdsale: new closing time is in the pass");
